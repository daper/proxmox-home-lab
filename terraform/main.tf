...
locals {
  ...
  proxmox_iso_datastore = data.proxmox_virtual_environment_datastores.self.datastore_ids[[
    for idx, content_type in data.proxmox_virtual_environment_datastores.self.content_types :
    idx if contains(content_type, "iso")
  ][0]]
  # picks the first images datastore
  proxmox_images_datastore = data.proxmox_virtual_environment_datastores.self.datastore_ids[[
    for idx, content_type in data.proxmox_virtual_environment_datastores.self.content_types :
    idx if contains(content_type, "images")
  ][0]]
}
